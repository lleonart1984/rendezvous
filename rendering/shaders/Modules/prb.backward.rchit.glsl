...
BIND_TENSOR(2, 0, vec3, 12, Parameters);
BIND_TENSOR(2, 1, vec3, 12, dParameters);
...
layout(set = 2, binding = 2) uniform Constants {
    ivec2 param_dim;
} consts;
...
void main() {
    ...
    if (instance.MaterialIndex == -2) { // Load material from parameters
        ...
        ivec2 coord = max(ivec2(0), min(consts.param_dim - 1, ivec2(C*consts.param_dim)));
        material.Diffuse = TENSOR_ELEMENT(Parameters, coord.x + coord.y*consts.param_dim.x);
    ...
    if (instance.MaterialIndex == -2) { // Update grad_parameters
        //vec3 dx = Payload.dL * Payload.L * Payload.PDF / Payload.BRDF_cos;/// Payload.PDF;// / max(vec3(0.0001), Payload.BRDF_cos);
        vec3 dx = Payload.dL * Payload.L * Payload.PDF * dot(V, Payload.Direction) / max(vec3(0.00001), Payload.BRDF_cos);/// Payload.PDF;// / max(vec3(0.0001), Payload.BRDF_cos);
        ivec2 coord = max(ivec2(0), min(consts.param_dim - 1, ivec2(C*consts.param_dim)));
        atomicAdd(TENSOR_ELEMENT(dParameters, coord.x + coord.y*consts.param_dim.x).x, dx.x);
        atomicAdd(TENSOR_ELEMENT(dParameters, coord.x + coord.y*consts.param_dim.x).y, dx.y);
        atomicAdd(TENSOR_ELEMENT(dParameters, coord.x + coord.y*consts.param_dim.x).z, dx.z);
    }
}