#version 460
#extension GL_GOOGLE_include_directive : require
#extension GL_EXT_scalar_block_layout : require


layout (local_size_x = 32, local_size_y = 32, local_size_z = 1) in;

layout(binding = 0, rgba32f) uniform image2D output_transmittance; // Output image

layout(binding = 1, r32f) uniform image3D grid; // Grid

struct RayDescription{
    vec3 Origin;
    vec3 Direction;
};
layout(scalar, binding = 2) buffer Rays { // Output rays generated by this shader
    RayDescription data[];
} rays;

layout(scalar, binding = 3) buffer Transmittance { // Output per-ray transmittances.
    vec3 data[]; // transmittance per component r,g,b
} transmittance;

layout(binding = 4) uniform CameraTransforms {
    mat4 ProjToWorld;
} camera;

layout(binding = 5) uniform MediumProperties {
    vec3 scatteringAlbedo;
    float density;
    float phase_g;
} medium;

// --- Common registers between functions
float g_density;
vec3 g_box_minim;
vec3 g_box_maxim;
vec3 g_box_size;
ivec3 g_dim;
vec3 g_cellSize;

void initialize_registers(){
    g_density = medium.density;
    g_dim = imageSize(grid);
	int maxDim = max(g_dim.x, max(g_dim.y, g_dim.z));
	g_box_maxim = g_dim * 0.5 / maxDim;
	g_box_minim = -g_box_maxim;
    g_box_size = g_box_maxim - g_box_minim;
    g_cellSize = g_box_size / g_dim;
}

bool rayBoxIntersect(vec3 bMin, vec3 bMax, vec3 P, vec3 D, out float tMin, out float tMax)
{
    // un-parallelize D
    D.x = abs(D).x <= 0.000001 ? 0.000001 : D.x;
    D.y = abs(D).y <= 0.000001 ? 0.000001 : D.y;
    D.z = abs(D).z <= 0.000001 ? 0.000001 : D.z;
    vec3 C_Min = (bMin - P)/D;
    vec3 C_Max = (bMax - P)/D;
	tMin = max(max(min(C_Min[0], C_Max[0]), min(C_Min[1], C_Max[1])), min(C_Min[2], C_Max[2]));
	tMin = max(0.0, tMin);
	tMax = min(min(max(C_Min[0], C_Max[0]), max(C_Min[1], C_Max[1])), max(C_Min[2], C_Max[2]));
	if (tMax <= tMin || tMax <= 0) {
		return false;
	}
	return true;
}

void CreateScreenRay(in vec2 screen_coord, out vec3 x, out vec3 w)
{
    vec2 coord = screen_coord * 2 - 1;

    vec4 ndcP = vec4(coord, 0, 1);
	ndcP.y *= -1;
	vec4 ndcT = ndcP + vec4(0, 0, 1, 0);

	vec4 viewP = camera.ProjToWorld * ndcP;
	viewP.xyz /= viewP.w;
	vec4 viewT = camera.ProjToWorld * ndcT;
	viewT.xyz /= viewT.w;

	x = viewP.xyz;
	w = normalize(viewT.xyz - viewP.xyz);
}

float RegularGridTransmittance(vec3 x, vec3 w, float total_d){
    int maxDim = max(g_dim.x, max(g_dim.y, g_dim.z));
    ivec3 cell = ivec3((x - g_box_minim) * g_dim / (g_box_maxim - g_box_minim));
    cell = clamp(cell, ivec3(0), g_dim - ivec3(1));
    vec3 alpha_inc = vec3(1) / (maxDim * max(vec3(0.000001), abs(w)));
	ivec3 side = ivec3(sign(w));
	vec3 corner = (cell + side * 0.5 + vec3(0.5)) * g_cellSize + g_box_minim;
	vec3 alpha = abs(corner - x) / max(vec3(0.000001), abs(w));
    float tau = 0;
    float current_t = 0;
    while(current_t < total_d){
	    float next_t = min(alpha.x, min(alpha.y, alpha.z));
		ivec3 cell_inc = ivec3(
			alpha.x <= alpha.y && alpha.x <= alpha.z,
			alpha.x > alpha.y && alpha.y <= alpha.z,
			alpha.x > alpha.z && alpha.y > alpha.z);

        tau += (next_t - current_t)*imageLoad(grid, cell).x * g_density;

		current_t = next_t;
		alpha += cell_inc * alpha_inc;
		cell += cell_inc * side;
    }

    return exp(-tau);
}


void main()
{
    initialize_registers();

    ivec2 dim = imageSize(output_transmittance);
    vec3 ray_origin, ray_direction;
    CreateScreenRay((gl_GlobalInvocationID.xy + 0.5) / dim, ray_origin, ray_direction);

    int index = int(gl_GlobalInvocationID.x + gl_GlobalInvocationID.y * dim.x);
    rays.data[index].Origin = ray_origin;
    rays.data[index].Direction = ray_direction;

    vec3 color ;
    float tMin, tMax;
    if (rayBoxIntersect(g_box_minim, g_box_maxim, ray_origin, ray_direction, tMin, tMax))
    {
        vec3 x = ray_origin + tMin * ray_direction;
        vec3 w = ray_direction;
        float total_d = tMax - tMin; // TOTAL DISTANCE INSIDE VOLUME
        color = vec3(1,1,1)*RegularGridTransmittance(x, w, total_d);
    }
    else
        color = vec3(1,1,1);

    imageStore( output_transmittance, ivec2(gl_GlobalInvocationID.xy), vec4( color, 1 ) );
    transmittance.data[index] = color;
}
