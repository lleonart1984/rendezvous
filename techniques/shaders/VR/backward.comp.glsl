...
layout(scalar, binding = 0) buffer Gradients{
    float data[];
} gradients; // Gradients
...
layout(scalar, binding = 2) readonly buffer Transmittance { // Per-ray computed transmittances.
    vec3 data[]; // transmittance per component r,g,b
} transmittance;

layout(scalar, binding = 3) readonly buffer dTransmittance { // Per-ray computed transmittances.
    vec3 data[]; // transmittance per component r,g,b
...
float RegularGridTransmittance(vec3 x, vec3 w, float total_d, vec3 T, vec3 dL_dT){
    int maxDim = max(g_dim.x, max(g_dim.y, g_dim.z));
...
void main()
{
    ...
    if (rayBoxIntersect(g_box_minim, g_box_maxim, ray_origin, ray_direction, tMin, tMax))
    {
        ...
        RegularGridTransmittance(x, w, total_d,
        transmittance.data[index],
        gradient_transmittance.data[index]);
    }
}
